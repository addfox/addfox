...
- **`cache/`** — Caché persistente de build (`cache/build/`), además de los profiles del navegador de dev (`cache/browser-profile/`) cuando `keepBrowserProfile` está habilitado

:::tip
Evita eliminar el directorio `cache` a menos que sea necesario, ya que acelera las compilaciones posteriores. Si habilitaste `keepBrowserProfile`, también contiene el profile del navegador de dev (sesiones de inicio de sesión, estado de la extensión).
:::
