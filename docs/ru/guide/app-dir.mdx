...
- **`cache/`** — постоянный кэш сборки (`cache/build/`), а также профили браузера разработки (`cache/browser-profile/`) при включенном `keepBrowserProfile`

:::tip
Не удаляйте директорию `cache` без необходимости — она ускоряет последующие сборки. Если включен `keepBrowserProfile`, в ней также хранится профиль браузера разработки (сессии входа, состояние расширения).
:::
