...
We are excited to share the latest progress on the AddFox framework. While the current npm stable release remains at `0.1.1`, the `dev-launch` branch has undergone a significant architecture refactoring. This update focuses on three core goals: **reducing dependencies**, **replacing black-box tools with first-party solutions**, and **improving the developer experience** for browser extension development.

## Dependency Slimming & Supply Chain Security

One of the most impactful changes in this refactoring is the aggressive removal of third-party dependencies that carried large transitive trees. Every external dependency is a potential supply-chain risk—whether through deprecation, malicious takeover, or breaking changes in transitive packages. By replacing large third-party tools with focused, in-house utilities, we have drastically shrunk the attack surface for AddFox users.

Here is what we removed:

- **`web-ext`** (~586 KB + **26 transitive deps**) — completely removed.
- **`chrome-launcher`** — removed.
- **`archiver`** (~40 KB + **9 transitive deps**) — replaced with a custom zip utility.
- **`cli-table3`** (~46 KB + 1 transitive dep) — removed from `@addfox/core`.
- **`chalk`** (~44 KB), **`kolorist`**, **`minimist`** — removed from `create-addfox-app` and replaced with lightweight in-house utilities.
...
Previously, Firefox development relied on `web-ext run`, a powerful but heavy and opaque tool. We have built **`@addfox/launcher`** from the ground up:
...
These changes are currently on the `dev-launch` branch and will be published as a new version soon. Given the scope of the architectural changes—especially the replacement of the HMR engine—we expect the next release to be at least a minor version bump, likely `0.2.0`.

Stay tuned, and thank you for using AddFox!
