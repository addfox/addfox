...
- **`cache/`** — Persistent build cache (`cache/build/`), plus dev browser profiles (`cache/browser-profile/`) when `keepBrowserProfile` is enabled

:::tip
Avoid deleting the `cache` directory unless necessary — it speeds up subsequent builds. If you enabled `keepBrowserProfile`, it also holds the dev browser profile (login sessions, extension state).
:::
